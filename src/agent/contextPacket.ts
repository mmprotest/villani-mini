...
export function buildContextPacket(i:ContextPacketInput){
  return JSON.stringify({
    ...
    pendingUserQuestion:i.pendingUserQuestion||null,pendingApproval:i.pendingApproval||null,recoveryHint:i.recoveryHint||null,noProgressSummary:i.noProgressSummary||null,repeatedFailureSummary:i.repeatedFailureSummary||null,discouragedActions:(i.discouragedActions||[]).slice(-6),bannedNextActions:(i.bannedNextActions||[]).slice(-3),recoveryInstruction:i.recoveryInstruction||null,
    allowedActions:i.allowedActionTypes,stopRules:i.stopRules||['completed','blocked','waiting_for_approval','waiting_for_user','budget_exhausted'],safetyRules:['return exactly one JSON action object','smallest useful next action','prefer read/inspect when uncertain','do not repeat failed action','avoid stale repeated read_current_page results','use ask_user for missing info','use final_answer blockedReason when blocked','include snapshotId for browser actions','never invent evidence']
  });
}
