...
export function buildContextPacket(i:ContextPacketInput){
  return JSON.stringify({
    ...
    pendingUserQuestion:i.pendingUserQuestion||null,pendingApproval:i.pendingApproval||null,recoveryHint:i.recoveryHint||null,
    allowedActions:i.allowedActionTypes,stopRules:i.stopRules||['completed','blocked','waiting_for_approval','waiting_for_user','budget_exhausted'],safetyRules:['return exactly one JSON action object','smallest useful next action','prefer read/inspect when uncertain','do not repeat failed action','use ask_user for missing info','use final_answer blockedReason when blocked','include snapshotId for browser actions','never invent evidence']
  });
}
