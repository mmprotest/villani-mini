...
export class AgentController {
  ...
  private redactActionParams(action: AgentAction, riskReasons: string[]): Record<string, unknown> {
    const params = { ...((action.params ?? {}) as Record<string, unknown>) };
    if (action.type === 'fill_field' && riskReasons.some((r) => r.includes('sensitive_field'))) params.value = '[REDACTED]';
    return params;
  }
  private makeRecord(taskId:string, action:AgentAction): ActionRecord { const now=new Date().toISOString(); const riskLevel=action.meta?.riskLevel??scoreRisk(JSON.stringify(action),'low'); const evalResult=evaluateActionPermission(action.type,(action.params ?? {}) as Record<string, unknown>,riskLevel,{snapshot:this.browser.getCurrentSnapshot() ?? undefined}); const redactedParams=this.redactActionParams(action, evalResult.riskReasons); return {id:`p_${Date.now()}_${Math.random()}`,taskId,type:action.type,params:redactedParams,title:action.meta?.title??action.type,reason:action.meta?.reason??'proposed',expectedOutcome:action.meta?.expectedOutcome??'progress',riskLevel,requiresApproval:evalResult.requiresApproval,reversible:action.meta?.reversible??true,evidenceRefs:action.meta?.evidenceRefs??(Array.isArray((action.params as any).evidenceRefs)?(action.params as any).evidenceRefs:[]),createdAt:now,updatedAt:now,status:'proposed',approvalDetails:{targetSummary:evalResult.targetSummary,riskReasons:evalResult.riskReasons,snapshotId:(this.browser.getCurrentSnapshot()?.snapshotId),candidateId:typeof (action.params as any)?.candidateId === 'string' ? (action.params as any).candidateId : undefined,fieldId:typeof (action.params as any)?.fieldId === 'string' ? (action.params as any).fieldId : undefined},}; }
  private async persistProposalAndMaybeExecute(taskId:string, action:AgentAction){ const riskLevel=action.meta?.riskLevel??scoreRisk(JSON.stringify(action),'low'); const evalResult=evaluateActionPermission(action.type,(action.params ?? {}) as Record<string, unknown>,riskLevel,{snapshot:this.browser.getCurrentSnapshot() ?? undefined}); if(!evalResult.canExecute){ this.event(taskId,'action_validation_failed',`${action.type}: ${evalResult.failureReason ?? 'permission evaluation failed'}`); this.store.updateTask(taskId,{status:'idle',pendingProposalId:null,recoveryHint:evalResult.failureReason ?? 'Action target unavailable. Read current page and retry.'}); return this.getTaskState(taskId); } const proposal=this.makeRecord(taskId, action); this.store.appendAction(taskId,proposal); this.event(taskId,'model_action_proposed',proposal.type,proposal.id);
  ...
  async executeProposal(taskId:string,proposalId:string){ this.store.updateAction(taskId,proposalId,{status:'executing'}); const a:any=this.store.getAction(taskId,proposalId); this.event(taskId,'action_started',a.type,proposalId); let out: ActionExecutionResult;
    ...
    if(a.type==='ask_user'&&out.ok){ this.store.updateTask(taskId,{status:'waiting_for_user',pendingUserQuestion:{question:a.params.question,options:a.params.options||[]}}); this.event(taskId,'user_question',a.params.question); }
