...
export class AgentController {
  ...
  private makeRecord(taskId:string, action:AgentAction): ActionRecord { const now=new Date().toISOString(); const evalResult = evaluateActionPermission(action.type, (action.params ?? {}) as Record<string, unknown>, 'low', { snapshot: this.browser.getCurrentSnapshot() }); return {id:`p_${Date.now()}_${Math.random()}`,taskId,type:action.type,params:(action.params ?? {}) as Record<string, unknown>,title:action.meta?.title??action.type,reason:action.meta?.reason??'proposed',expectedOutcome:action.meta?.expectedOutcome??'progress',riskLevel:action.meta?.riskLevel??scoreRisk(JSON.stringify(action),'low'),requiresApproval:evalResult.requiresApproval,reversible:action.meta?.reversible??true,evidenceRefs:action.meta?.evidenceRefs??(Array.isArray((action.params as any).evidenceRefs)?(action.params as any).evidenceRefs:[]),createdAt:now,updatedAt:now,status:'proposed',approvalDetails:{targetSummary:evalResult.targetSummary,riskReasons:evalResult.riskReasons,snapshotId:this.browser.getCurrentSnapshot()?.snapshotId,candidateId:typeof (action.params as any)?.candidateId==='string'?(action.params as any).candidateId:undefined,fieldId:typeof (action.params as any)?.fieldId==='string'?(action.params as any).fieldId:undefined}}; }
