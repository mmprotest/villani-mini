...
export default function HomeView() {
  ...
  useEffect(() => {
    ...
    window.villani.setup?.getStatus?.().then?.(setSetupStatus);
    void loadTasks();
    void loadBrowser();
    void loadConfig();
  ...
  return <div className='app-shell'>
    ...
    <section className='main'>
      ...
      {view === 'Home' && <>
        ...
        <div className='panel'>{messages.length === 0 && ready ? <p>Villani Mini is ready. Ask for managed browser steps or approved local actions.</p> : messages.slice(-8).map((m) => {
          ...
          if (m.status === 'waiting_for_user') {
            ...
            return <div key={m.id} className={`msg ${msgType}`}><b>Question</b><div>{m.text || m.content}</div><div className='row-actions'><input value={text} onChange={(e)=>setText(e.target.value)} placeholder='Your answer' /><button disabled={missing || sending || !text.trim()} onClick={async()=>{ if(!m.taskId) return; setSending(true); setResponseError(''); try { const out = await window.villani.chat.answer(m.taskId, text.trim()); setMessages(out); setText(''); } catch (e:any) { setResponseError(String(e?.message||e)); } finally { setSending(false);} }}>Submit</button></div>{missing && <div className='subtle'>Missing metadata: taskId/questionId.</div>}</div>;
          }
          return <div key={m.id} className={`msg ${msgType}`}><b>{m.status || m.type || m.role}</b><div>{m.text || m.content}</div></div>;
        })}{responseError && <p className='subtle'>{responseError}</p>}</div>
      </>}
      ...
      {view === 'Commands' && <div className='panel'><h2>Commands</h2>{Object.entries(actionCatalog).map(([k, vals]) => <div key={k}><h3>{k}</h3>{vals.map((v)=><div className='msg' key={v.name}><b>{v.name}</b> · default: {v.approval}<div className='subtle'>{v.desc}</div></div>)}</div>)}</div>}
