...
describe('ipc chat handlers', () => {
  ...
  it('forwards chat:updated from chat controller subscription', () => { const cb = mocks.onUpdated.mock.calls[0][0]; cb([{ id: 'm1' }]); expect(mocks.send).toHaveBeenCalledWith('chat:updated', [{ id: 'm1' }]); });
});
